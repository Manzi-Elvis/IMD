...
// Mirrors backend/src/incidents/incident-state-machine.ts. The frontend
// never enforces this on its own — the dropdown just hides invalid options
// for a better UX; the backend is what actually rejects an illegal
// transition if this list ever drifts out of sync.
...
// Human-readable verb for non-comment events; comments render their content
...
export function IncidentDetailPage() {
  ...
  const loadAll = async (incidentId: string) => {
    const [inc, tl, atts, allUsers] = await Promise.all([
      incidentsApi.getOne(incidentId),
      incidentsApi.getTimeline(incidentId),
      incidentsApi.listAttachments(incidentId),
      usersApi.list(),
    ]);
    setIncident(inc);
    setTimeline(tl);
  ...
  // Real-time: any responder viewing this incident sees status/severity/
  // owner changes and new timeline events the moment another user makes
  // them, without polling or refreshing.
  useIncidentSocket({
    incidentId: id ?? null,
    token,
  ...
  const handleComment = async (e: FormEvent) => {
    ...
    try {
      ...
      setComment('');
      // Socket push handles appending it to the timeline for everyone,
      // including us, so we don't optimistically append here too.
    } catch (err: any) {
  ...
  const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const attachment = await incidentsApi.uploadAttachment(incident.id, file);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Reported by {incident.reporter.name} on{' '}
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          {canManage && NEXT_STATES[incident.status].length > 0 ? (
            <select
              value=""
              onChange={(e) =>
                e.target.value && handleStatusChange(e.target.value as IncidentStatus)
              }
      ...
      <div className="mb-6 rounded-lg border border-slate-200 bg-white p-4">
        <h3 className="mb-3 text-sm font-medium text-slate-500">Evidence & attachments</h3>
        <div className="mb-3 space-y-2">
          ...
          {attachments.map((att) => (
            <a
              key={att.id}
              href={incidentsApi.downloadAttachmentUrl(incident.id, att.id)}
              className="flex items-center justify-between rounded-md border border-slate-100 px-3 py-2 text-sm hover:bg-slate-50"
            >
              <span className="font-medium text-slate-800">{att.originalName}</span>
              <span className="text-slate-400">
                {(att.sizeBytes / 1024).toFixed(0)} KB · {att.uploadedBy.name}
              </span>
            </a>
          ))}
        </div>
